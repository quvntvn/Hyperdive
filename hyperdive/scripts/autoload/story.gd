...
# Autoload "Story" — campagne narrative de 21 chapitres jouables (l'histoire de Hyperdive).
...
#
# Textes définitifs (21 chapitres, TOUS jouables : 15 chute, 6 jetpack). PLUS AUCUN chapitre
# purement narratif — chaque ancien nœud type:"story" a été fusionné VERBATIM dans l'outro
# (text_when "after") du niveau jouable qui le précédait. Le texte d'un chapitre = [outro du
# niveau] + ligne(s) vide(s) + narration(s) absorbée(s), dans l'ordre.
# Déblocages liés à la complétion : ch.1 → mode Classique, ch.11 → mode Jetpack (voir complete_chapter).
# (ch.11 = dernier niveau chute, son outro contient le beat « L'inversion » → juste avant le 1er
# niveau jetpack ch.12 : mécanique et récit alignés.)
...
const CHAPTERS: Array[Dictionary] = [
	...
	# Outro = « Résister » + « La faille » (ancien ch.18).
	{"n": 10, "type": "play",  "mode": "fall",    "objective": {"kind": "dodge",    "value": 30},   "text_when": "after",  "theme": "simulation", "title": "Résister", "image": "res://assets/story/ch10.png",
		"text": "Mais à force de tomber, j'apprends.\nLe vent répond à mes gestes. Quand je me concentre, la chute hésite — d'un quart de seconde, d'un souffle. La simulation n'est pas parfaite. Elle attend que je subisse, et tant que je subis, elle me tient.\nEt si je cessais de subir ?\n\nJe l'ai sentie cette nuit — s'il y a des nuits ici.\nLa chute n'est pas une loi. C'est une consigne. Quelqu'un a écrit « il tombe », et l'univers obéit. Mais une consigne, ça se réécrit.\nLe bas n'est pas une fatalité. Ce n'est qu'une direction. Et toute direction a un contraire."},
	# ch.11 = DÉBLOCAGE JETPACK à la complétion. Outro = « Le refus » + « L'inversion » (ancien ch.20,
	# le beat de bascule chute → montée) : juste avant le 1er niveau jetpack ch.12.
	{"n": 11, "type": "play",  "mode": "fall",    "objective": {"kind": "survive",  "value": 44},   "text_when": "after",  "theme": "aube", "title": "Le refus", "image": "res://assets/story/ch11.png",
...
# Tous les chapitres sont désormais jouables. Formule : 40 + n*10 (croissante, récompense les
# chapitres tardifs). Calibrée pour que le TOTAL campagne (21 ch.) reste ≈ l'ancien total des 40
# chapitres : avant = jouables Σ(40+n*5) + 19 narrations ×15 = 2900 + 285 = 3185 pièces ;
# après = Σ_{n=1..21}(40+n*10) = 40×21 + 10×231 = 840 + 2310 = 3150 pièces (≈, −35).
...
# de mode (CLASSIQUE au ch.1, JETPACK au ch.11) et avance la frontière de progression d'un cran.
...
func complete_chapter(n: int) -> int:
	...
	if n == 11:
		Settings.jetpack_unlocked = true    # JETPACK débloqué à la fin du chapitre 11 (dernier
		                                    # niveau chute, son outro porte le beat « L'inversion »)
