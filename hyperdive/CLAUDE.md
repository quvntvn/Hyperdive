...
- **Victoire campagne = FONDU NOIR** (parachute SUPPRIMÉ — ne collait plus au ton) : objectif atteint → `player.complete_run()` (fige le run, invincible, le perso file ~0,5 s) → `Transition.fade_to_black()` → écran de victoire pendant `fade_from_black()`. Fondus manuels ajoutés à l'autoload Transition (verrou partagé avec change_scene).
...
- **Objectifs** : `distance` (atteindre value m), `survive` (survivre value s), `dodge` (esquiver value obstacles via `Story.dodged`/`notify_dodge`), `descent` (OUVERTURE ch.1 : MOURIR = réussir — implémenté, bullet suivant).
- **Ouverture ch.1 "descent"** (étape 4, EN PLACE) : chute scriptée SANS obstacles/power-ups/pièces ni musique (whoosh seul, `Audio.stop_music`, relancée au menu). `value` = distance du sol (400 m ≈ 23 s à 18 m/s). `main_game._setup_descent` : SOL = StaticBody3D 60×60 sombre, groupe "obstacles", top exactement à -value → mort par le chemin de collision EXISTANT, inévitable ; 6 silhouettes famille (`story_fallers.gd` : capsules sombres unshaded SANS collision, suivent le Y interpolé du joueur, se dispersent une à une à 18→86 % de la descente — enfants d'abord, parents en dernier, le joueur finit seul) ; HUD = ALTIMÈTRE décroissant ("400 m" → "0 m" via `update_story_progress`, orange ≤ 50 m via `set_story_progress_urgent`). La mort est routée en RÉUSSITE par `player.gd` (branche descent : ragdoll ~1,5 s → `pending_outro` + `complete_chapter` → fondu noir → outro "2028" → carte). PAS d'écran "CHAPITRE RÉUSSI", pas de jingle game over (silence voulu). Le ch.1 ne peut PAS échouer. `game_over` est émis à TOUTE mort campagne (stoppe le driver — évitait une réussite "survive" posthume pendant le ragdoll).
...
- **CARTE** (`campaign_screen.gd`) : ÉCRAN PLEIN en verre, chemin vertical en zig-zag entre les **2 tours de Vertex** (calque de fond, shader `tower_windows.gdshader`, teinté par le thème), 40 nœuds construits en code. TITRE sous CHAQUE nœud débloqué (`TITLE_W = 170` calé entre les tours, autowrap 2 lignes, liseré sombre ; courant un peu plus grand/lumineux, complétés crème adoucie) — les verrouillés n'affichent PAS leur titre (mystère). Nœud débloqué : numéro en HAUT + logo de type DESSOUS. Nœud courant glow, complétés badge ✓, verrouillés cadenas (`lock_icon.svg`). Couleur nœud = type (crème narration, turquoise chute, jaune jetpack). Pivot ↓↑ au ch.20 (inversion). Bouton "Archives" (galerie) + retour. Ouverture centrée sur le chapitre courant.
- **Lecteur immersif** (`chapter_reader.gd`) : image plein fond (ou placeholder), voiles dégradés, titre + texte scrollable, fondu cinématique. Contextes : `story` (CONTINUER = complète + retour carte), `play_before` (JOUER = lance le niveau), `outro` (texte après victoire, lecture seule), `gallery` (relecture ◀▶ parmi les chapitres débloqués). **Indicateur de scroll** : sur les chapitres à texte long, deux signaux discrets construits en code (`_build_scroll_hint`) signalent qu'on peut scroller — un FONDU vers le sombre plaqué sur le bas de la zone de texte + un chevron ▼ à pulsation douce dessous. Ils ne sont visibles que s'il reste du contenu sous le bord et s'effacent dès qu'on atteint le bas (`_update_scroll_hint`, branché sur la scrollbar). Valable dans TOUS les contextes (narration/intro/outro/galerie).
...
- **Driver d'objectif** (`main_game.gd`, quand `Story.active`) : `_process` lit l'objectif, calcule la progression (distance/survie/esquive), pousse le HUD (`update_story_progress` → "320 / 800 m", "12 / 24 s", "8 / 15 esquives") et déclenche la réussite à `cur >= value`. Pour `descent` : pousse l'ALTIMÈTRE décroissant, AUCUNE réussite en vol (seul le sol termine). Pièces OFF + power-ups sans aimant (même setup que coop).
- **Victoire** (`chapter_end_screen.gd`, état "victory") : `complete_run()` + fondu noir (~0,5 s de chute libre avant) → `Story.complete_chapter(n)` → écran verre "CHAPITRE RÉUSSI" + pièces gagnées + CONTINUER. Flux : si `text_when=="after"` → pose `Story.pending_outro` → retour menu lit l'outro dans le lecteur puis carte ; sinon retour carte direct.
- **Échec doux** (`chapter_end_screen`, état "failure") : la mort en campagne (`player.gd`, branche `Story.active`, SAUF ch.1 "descent" où mort = réussite) affiche "TU ES TOMBÉ" + RÉESSAYER (reload même chapitre) + RETOUR CAMPAGNE. Retry ILLIMITÉ, aucune stat, aucune perte, aucun record. Le chapitre n'est complété QUE sur objectif atteint (en jeu), jamais à l'échec.
...
- **Gameplay** : `scripts/gameplay/{follow_camera, obstacle_spawner, coin_spawner, corridor_walls, obstacle_base, obstacle_door, obstacle_oscillating, obstacle_bar, obstacle_wall, powerup_spawner, story_fallers, main_game}.gd`
